...
router.put('/task/:taskId/stage/:stageId', authMiddleware, async (req, res) => {
  try {
    ...
    // التحقق من الموافقات المطلوبة قبل النقل حسب نوع المرحلة
    const approvalType = targetStage.approval_type || 'multiple';
    const hasAdmin = !!task.approved_by_admin;
    const hasMain = !!task.approved_by_main_lawyer;
    const hasAssigned = !!task.approved_by_assigned_lawyer;
    ...
    if (approvalType === 'admin_only') {
      ...
      if (!(hasAdmin || hasMain || hasAssigned)) {
        return res.status(400).json({ message: 'يجب الحصول على موافقة واحدة على الأقل' });
      }
    } else {
      if (!(hasAdmin && hasMain && hasAssigned)) {
        return res.status(400).json({ message: 'يجب اكتمال موافقات الإدارة والمحامين قبل النقل' });
      }
    }
...
router.post('/:stageId/tasks/:taskId/approve', authMiddleware, async (req, res) => {
  try {
    const taskId = parseInt(req.params.taskId);
    const task = inMemoryDB.tasks.get(taskId);
    ...
    const assignedLawyerId = task.assigned_lawyer_id || task.assigned_to;
    const mainLawyerId = task.main_lawyer_id || task.main_lawyer;

    // تحديث حالة الموافقة بناءً على دور المستخدم
    if (req.user.role === 'admin') {
      updatedTask.approved_by_admin = req.user.id;
      updatedTask.approved_at_admin = new Date();
    } else if (req.user.role === 'lawyer') {
      if (assignedLawyerId === req.user.id) {
        updatedTask.approved_by_assigned_lawyer = req.user.id;
        updatedTask.approved_at_assigned_lawyer = new Date();
      } else {
        updatedTask.approved_by_main_lawyer = req.user.id;
        updatedTask.approved_at_main_lawyer = new Date();
      }
    }
